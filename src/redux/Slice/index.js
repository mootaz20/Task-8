...
export const deleteProduct = createAsyncThunk("products/deleteProduct", async (id, { rejectWithValue }) => {
    try {
        const token = localStorage.getItem("token");
        const response = await axios.delete(`https://test1.focal-x.com/api/items/${id}`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
...
export const updateProduct = createAsyncThunk("products/updateProduct", async ({ data, id }, { rejectWithValue }) => {
    try {
        const token = localStorage.getItem("token");
        const response = await axios.post(`https://test1.focal-x.com/api/items/${id}`, data, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
...
export const addProduct = createAsyncThunk("products/addProduct", async (data, { rejectWithValue }) => {
    try {
        const token = localStorage.getItem("token");
        const response = await axios.post("https://test1.focal-x.com/api/items", data, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
...
const ProductsSlice = createSlice({
    ...
    reducers: {
        searchProducts: (state, action) => {
            state.products = state.AllProducts.filter((el) => el.name.toLowerCase().includes(action.payload.toLowerCase()));
